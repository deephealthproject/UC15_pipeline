...
struct Arguments {
  ...
  Arguments(std::string yaml_path, const std::vector<int> &target_shape,
            const int epochs, const int batch_size, const bool use_dldataset,
            const int workers, const std::vector<int> &gpus, const int lsb,
            const bool cpu, const std::string mem_level,
            const std::string augmentations, const std::string model,
            const std::string ckpt, const std::string optimizer,
      ...
      : yaml_path(yaml_path), target_shape(target_shape), epochs(epochs),
        batch_size(batch_size), use_dldataset(use_dldataset), workers(workers),
        gpus(gpus), lsb(lsb), cpu(cpu), mem_level(mem_level),
        augmentations(augmentations), model(model), ckpt(ckpt),
